...
import { ArrowRight, BookOpen, CheckCircle2, FileText, Video, Headphones } from 'lucide-react';
...
export function AulaVirtual() {
  ...
  const gt = glossaryTranslations[language] || glossaryTranslations.es;
  ...
  const recursos = [
    { name: gt.lecturas, icon: <FileText size={20} className="text-[#00823B]" />, path: '/recursos/lecturas' },
    { name: gt.videos, icon: <Video size={20} className="text-[#00823B]" />, path: '/recursos/videos' },
    { name: gt.audios, icon: <Headphones size={20} className="text-[#00823B]" />, path: '/recursos/audios' },
    { name: gt.pageTitle, icon: <BookOpen size={20} className="text-[#00823B]" />, path: '/glosario' },
  ];
      ...
      <section className="py-4 sm:py-10 lg:py-16 bg-[#FAF9F6]">
        <div className="px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
          ...
          <div className="mb-8 sm:mb-12">
            <h2 className="text-xs sm:text-sm font-black text-[#192A56]/60 uppercase tracking-[0.2em] mb-4">
              {t.quickResourcesTitle}
            </h2>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {recursos.map((rec, idx) => (
                <Link
                  key={idx}
                  to={rec.path}
                  className="group flex items-center gap-3 p-4 bg-white rounded-2xl border border-stone-200 hover:border-[#00823B] shadow-sm hover:shadow-md transition-all duration-300 active:scale-[0.99]"
                >
                  <div className="shrink-0 transition-transform group-hover:scale-110">
                    {rec.icon}
                  </div>
                  <span className="text-sm sm:text-base font-bold text-[#192A56] group-hover:text-[#00823B] transition-colors">
                    {rec.name}
                  </span>
                </Link>
              ))}
          ...
          <div className="space-y-3 sm:space-y-5">
            ...
            {/* Malaga Resources Card at the end of the lessons */}
            <Link
              to="/recursos-malaga"
              className="group block bg-[#00823B]/5 rounded-2xl sm:rounded-3xl border border-[#00823B]/20 shadow-sm hover:shadow-xl hover:border-[#00823B]/30 transition-all duration-300 overflow-hidden active:scale-[0.98]"
            >
              <div className="flex flex-row">
                {/* Badge area */}
                <div className="w-20 sm:w-40 md:w-48 bg-gradient-to-br from-[#00823B] to-[#006A30] relative overflow-hidden flex-shrink-0 flex flex-col items-center justify-center py-5 sm:py-8">
                  <div className="absolute -left-4 -bottom-4 w-16 h-16 bg-white/5 rounded-full"></div>
                  <span className="text-3xl sm:text-5xl md:text-6xl font-black text-white drop-shadow-sm relative z-10">
                    📍
                  </span>
                </div>
                ...
                <div className="flex-1 p-3 sm:p-6 md:p-8 flex flex-col justify-center min-w-0">
                  <div className="mb-2 sm:mb-4">
                    <div className="flex items-start justify-between gap-2">
                      <h2 className="text-sm sm:text-xl md:text-2xl font-black text-[#192A56] mb-0.5 sm:mb-2 leading-tight group-hover:text-[#00823B] transition-colors">
                        {t.malagaTitle}
                      </h2>
                      <div className="hidden sm:flex items-center gap-2 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                        <span className="text-xs font-bold text-[#00823B] uppercase tracking-wider">{t.startButton}</span>
                    ...
                    <p className="text-stone-500 text-[10px] sm:text-sm leading-relaxed">
                      {t.malagaDesc}
                    </p>
                  </div>

                  <div className="flex items-center justify-between pt-2 sm:pt-3 border-t border-[#00823B]/10 gap-2">
                    <span className="text-[9px] sm:text-xs font-bold text-[#00823B] uppercase tracking-wider whitespace-nowrap">
                      {language === 'es' ? 'Guía de integración local' : 'Local integration guide'}
                    </span>
                    <div className="sm:hidden flex items-center gap-1">
                      <ArrowRight size={14} className="text-[#00823B]" />
                    </div>
